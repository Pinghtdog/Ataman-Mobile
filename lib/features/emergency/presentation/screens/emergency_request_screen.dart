...
class _EmergencyRequestScreenState extends State<EmergencyRequestScreen> with TickerProviderStateMixin {
    ...
    return Scaffold(
      ...
      body: BlocConsumer<EmergencyCubit, EmergencyState>(
        listener: (context, state) {
          ...
          } else if (state is EmergencyOffline) {
             _resetAnimation();
            _showOfflineOption(state.smsPayload);
          } else if (state is EmergencyQueued) {
             _resetAnimation();
             ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text("Emergency queued locally. Waiting for internet...")),
            );
  ...
  void _showOfflineOption(String smsPayload) {
      ...
      builder: (context) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text("You are Offline", style: AppTextStyles.h2),
            const SizedBox(height: 8),
            const Text(
              "We couldn't reach the server. Your emergency request is saved locally. Would you like to send an SOS via SMS instead?",
              textAlign: TextAlign.center,
              style: AppTextStyles.bodyMedium,
            ),
            const SizedBox(height: 24),
            AtamanButton(
              text: "SEND SMS SOS (FREE)",
              color: AppColors.danger,
              onPressed: () {
                Navigator.pop(context);
                context.read<EmergencyCubit>().sendSmsSos(smsPayload);
              },
            ),
