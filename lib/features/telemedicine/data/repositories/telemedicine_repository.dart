...
class TelemedicineRepository implements ITelemedicineRepository {
  ...
  Stream<List<DoctorModel>> watchDoctors() {
    return _supabase
        ...
        .handleError((error) {
          debugPrint('Doctor Stream Network Error (Suppressed): $error');
        });
  }
    ...
    return _supabase
        ...
        .map((data) => data.where((session) {
              ...
              if (scheduledTimeStr != null) {
                ...
                return scheduledTime.isAfter(startOfToday) || 
                       scheduledTime.isAtSameMomentAs(startOfToday);
              }
              
              return true;
            }).toList())
        .handleError((error) {
          debugPrint('Session Stream Network Error (Suppressed): $error');
        });
  }
  ...
  Stream<List<Map<String, dynamic>>> watchCall(String callId) {
    return _supabase
        ...
        .eq('id', callId)
        .handleError((error) {
          debugPrint('Call Stream Network Error (Suppressed): $error');
        });
  }
  ...
  Future<String> initiateCall(String patientId, String doctorId, {Map<String, dynamic>? metadata, DateTime? scheduledTime}) async {
     final Map<String, dynamic> insertData = {
      ...
      'started_at': scheduledTime == null ? DateTime.now().toUtc().toIso8601String() : null,
      'scheduled_time': scheduledTime?.toUtc().toIso8601String(), // Ensure UTC for DB
    };
 ...
 @override
  Future<bool> checkBookingConflict(String patientId, String doctorId, DateTime startOfDay, DateTime endOfDay) async {
    final response = await _supabase
        ...
        .lte('scheduled_time', endOfDay.toUtc().toIso8601String())
