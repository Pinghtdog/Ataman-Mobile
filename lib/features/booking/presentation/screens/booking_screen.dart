  ...
  List<Facility> _filterFacilities(List<Facility> facilities) {
    ...
    if (widget.triageResult != null) {
      ...
      filtered = filtered.where((f) {
        if (requiredCap == 'HOSPITAL_LEVEL_1' || requiredCap == 'HOSPITAL_LEVEL_2' || requiredCap == 'HOSPITAL_LEVEL_3') {
          return f.type == FacilityType.hospital;
        }
        if (requiredCap == 'BARANGAY_HEALTH_STATION' || requiredCap == 'BHS') {
                      ...
                      child: state is FacilityLoading
                        ...
                        : state is FacilityError
                          ...
                          : state is FacilityLoaded
                            ? (_isMapView && ModalRoute.of(context)?.isCurrent == true)
                              ? RepaintBoundary(
                                  child: AtamanLiveMap(
